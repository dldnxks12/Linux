	...
	if (pFile != NULL){
		...
		while(!feof(pFile)){
			...
			if(feof(pFile))
				break;
			printf("%s", strTemp);
			//write(fd, "START,0,R,010,D,1,L,150,D,1,Z", sizeof("START,0,R,010,D,1,L,150,D,1,Z"));
			//sleep(1);
			//write(fd, "START,0,R,150,D,1,L,100,D,1,Z", sizeof("START,0,R,010,D,1,L,150,D,1,Z"));
			write(fd, strTemp, 30);
			sleep(1);
		}
		fclose(pFile);
