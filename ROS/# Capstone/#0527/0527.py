...
try:
	while True:
		...
		if lines is not None:
			for i in lines:
				...
				if R < 0.5236 / 2: # 30 degree
					...
					result = R * (180 / math.pi) # rad to degree
					print(f"Degree : {result} || Right : {right_flag} || Left : {left_flag}")
					cv2.line(canvas[:][height:][:], (i[0][0], i[0][1]), (i[0][2], i[0][3]), (0, 0, 255), 2, cv2.LINE_AA)
					
					break # Only 1 line for frame  ...
		...
		if Start_flag == 1:
			logger.info(f"START,0,R,{R},D,1,L,100,D,1,Z")
