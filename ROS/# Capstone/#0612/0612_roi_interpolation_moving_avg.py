...
try:
	while True:
		...
		if lines is not None:
			Interpolation = lines # for interpolation flag 
			No_line_flag = 0             
			...
			#print("Left_Lines  & shape" , L_lines, L_lines.shape)
			#print("Right_Lines & shape", R_lines, R_lines.shape)
			...
			if len(L_lines) > 0:
				...
				if len(L_x1) < 10:
					...
					x1 = int(sum(L_x1[-10:]) / 10)
					y1 = int(sum(L_y1[-10:]) / 10)
					x2 = int(sum(L_x2[-10:]) / 10)
					y2 = int(sum(L_y2[-10:]) / 10)
					Left_Avg_points_temp = [x1, y1, x2, y2]
			...
			if len(R_lines) > 0:
				...
				if len(R_x1) < 10:
					...
					x1 = int(sum(R_x1[-10:]) / 10)
					y1 = int(sum(R_y1[-10:]) / 10)
					x2 = int(sum(R_x2[-10:]) / 10)
					y2 = int(sum(R_y2[-10:]) / 10)
					Right_Avg_points_temp = [x1, y1, x2, y2]

				cv2.line(canvas, (x1, y1), (x2, y2), (255,255,255), 2)	
		
		elif (lines is None) and (Interpolation is not None): 
			No_line_flag += 1
			if No_line_flag >= 5:
				pass # Go straing ... or something else..
			else: # Interpolation for 5 frames  ... 				
