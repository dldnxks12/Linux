...
# Record Video
out = cv2.VideoWriter('Record0517.avi', cv2.VideoWriter_fourcc('M','J','P','G'), 10, (IMG_WIDTH, IMG_HEIGHT))
...
while True:
	...
	# Remove Vertical Lines with Sobel filter
	canvas_origin = canvas.copy() * 0
	canvas      = cv2.cvtColor(canvas, cv2.COLOR_BGR2GRAY)

	lower_canvas = canvas[:][height:]
	lower_canvas = cv2.Canny(lower_canvas, 50, 200, None, 3) # Edge detect for only lower Image
	#lower_canvas = cv2.Sobel(lower_canvas, -1, 1, 0)

	lines       = cv2.HoughLinesP(lower_canvas, 1, np.pi/180, 50, None, 50, 10)
	if lines is not None:
		for i in lines:
			cv2.line(canvas_origin[:][height:][:], (i[0][0], i[0][1]), (i[0][2], i[0][3]), (0, 0, 255), 1, cv2.LINE_AA)
			#pts.append([ [i[0][0], i[0][1]], [i[0][2], i[0][3]] ])

	# Middle Line
	cv2.line(canvas_origin, (240, 272), (240, 136), (150,150,150), 1, cv2.LINE_AA)

	# Make video
	out.write(canvas_origin)
